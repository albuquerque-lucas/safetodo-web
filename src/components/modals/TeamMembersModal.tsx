...
type TeamMembersModalProps = {
  ...
  pendingMembers: { id: number; name: string; isManager: boolean }[]
  setPendingMembers: Dispatch<
    SetStateAction<{ id: number; name: string; isManager: boolean }[]>
  >
...
const TeamMembersModal = ({
  ...
  const [pendingRemoveTimer, setPendingRemoveTimer] = useState<number | null>(null)
  ...
  const armRemoval = (memberId: number) => {
    if (pendingRemoveId === memberId) {
      setPendingMembers((current) => current.filter((item) => item.id !== memberId))
      setPendingRemoveId(null)
      if (pendingRemoveTimer) {
        window.clearTimeout(pendingRemoveTimer)
        setPendingRemoveTimer(null)
      }
      return
    }
                    ...
                    <tr
                      ...
                      className={
                        pendingRemoveId === member.id
                          ? 'table-row-pending-remove'
                          : undefined
                      }
                      onClick={() => armRemoval(member.id)}
                      role="button"
                    >
                      <td className="table-row-remove-hint">{member.name}</td>
